<template>
      ...
      <table class="min-w-full divide-y divide-gray-200 bg-gray-800 text-white">
        ...
        <tbody class="bg-gray-700 divide-y divide-gray-600">
          <tr v-for="(employee, index) in job.employees" :key="employee.identifier">
            ...
            <td class="px-4 py-2">
              <button @click="promoteEmployee(employee.identifier)" class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600" :disabled="!canPromote(employee)">Promote</button>
              <button @click="demoteEmployee(employee.identifier)" class="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600" :disabled="!canDemote(employee)">Demote</button>
  ...
  export default {
    ...
    methods: {
      ...
      canPromote(employee) {
        const currentGradeIndex = this.job.grades.findIndex(grade => grade.grade === employee.job_grade);
        return currentGradeIndex < this.job.grades.length - 1;
      },
      canDemote(employee) {
        const currentGradeIndex = this.job.grades.findIndex(grade => grade.grade === employee.job_grade);
        return currentGradeIndex > 0;
      }
    }
  };
  </script>
